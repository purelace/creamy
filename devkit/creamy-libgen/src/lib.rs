...
impl ProtocolLibrary {
    ...
    pub fn load_all(
        ...
        for entry in std::fs::read_dir(path).unwrap().filter_map(|result| {
            ...
            let definition = compile(&mut self.pool, &content).unwrap();
            self.inner.insert(
                definition.name().resolve(&self.pool).to_string(),
                definition,
            );
        }
...
impl Codegen {
    ...
    #[must_use]
    pub const fn library(&self) -> &ProtocolLibrary {
        &self.library
    }
